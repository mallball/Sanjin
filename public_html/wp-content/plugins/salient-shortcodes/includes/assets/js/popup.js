jQuery(document).ready(function($) {
  ...
  function initUpload(clone) {
    ...
    itemToInit.find('.nectar-add-btn').on('click',function( event ) {
      ...
      custom_file_frame.on( "select", function() {
        ...
        jQuery('.nectar-media-preview',activeFileUploadContext).attr('src', attachment.attributes.url);
        jQuery('#' + relid ).val(attachment.attributes.url).trigger('change');
        
        jQuery('.nectar-add-btn',activeFileUploadContext).hide();
        jQuery('.nectar-media-preview',activeFileUploadContext).show();
        jQuery('.nectar-remove-btn',activeFileUploadContext).show();
      });
    ...
    itemToInit.find('.nectar-remove-btn').on('click', function( event ) {
      var activeFileUploadContext = jQuery(this).parent();
      var relid = jQuery(this).attr('rel-id');
      ...
      jQuery('.nectar-media-preview',activeFileUploadContext).fadeOut('slow');
  ...
  $('.add-list-item').on('click', function() {
    ...
    if(!$(this).parent().find('.remove-list-item').is(':visible')) {
      $(this).parent().find('.remove-list-item').show();
    }
    ...
    if( $clone.find('.nectar-add-btn').length > 0 ) {
      
      $clone.find('.nectar-media-preview').attr('src','');
      $clone.find('.nectar-remove-btne').hide();
      $clone.find('.nectar-add-btn').css('display','inline-block');
      setTimeout(function() { 
        initUpload($clone);
      },200);
  ...
  $('body').on('keyup','.shortcode-dynamic-item-input, .shortcode-dynamic-item-text', function() { 
    dynamic_items(); 
  });
  $("body").on("input propertychange", '.shortcode-dynamic-item textarea', function() { 
    dynamic_items(); 
  });
  
  //icon selection
  $('.icon-option i').on('click', function() {
    $('.icon-option i').removeClass('selected');
    $(this).addClass('selected');
  });
  
  //icon set selection
  $('select[name="icon-set-select"]').on('change', function() {
  ...
  $('#options-icon input.enable_animation').on('change', function() {
    if($('#options-icon input.enable_animation').is(':checked')) {
      $('#options-icon input[data-attrname="animation_delay"], #options-icon select#animation_speed').parent().slideDown();
      $('#options-icon input[data-attrname="animation_delay"], #options-icon select#animation_speed').parent().next('.clear').slideDown();
      $('#options-icon input[data-attrname="animation_delay"], #options-icon select#animation_speed').parent().prev('.label').slideDown();
  ...
  //color change
  $('#options-icon #color').on('change', function() {
  ...
  $("#options-nectar_portfolio #category").on('change', function() {
    
    var selectedCats = $(this).val();
    
    if(selectedCats == 'all') {
      $('#options-nectar_portfolio #starting_category option')
        .removeAttr('disabled')
        .removeAttr('selected')
        .show();
    } else {
  ...
  function resetFileds() {
    ...
    $('#nectar-sc-generator').find('.nectar-media-preview').attr('src','');
    $('#nectar-sc-generator').find('.nectar-remove-btne').hide();
    $('#nectar-sc-generator').find('.nectar-add-btn').show();
